	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
				...
				$description  = !empty(get_field('mo_ta',$item->ID)) ? get_field('mo_ta',$item->ID) : '';
				...
				if($depth != 0 && $image_url && !$is_grid_display){
					$item_output .= '<img height="140px" src="'.$image_url.'" alt="'.$args->link_before.'"/>';
				}
				$item_output .= '<div class="hp-menu-title">'. $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after . '</div>';
				if($depth != 0 && $image_url && !$is_grid_display){
					$item_output .= '<div class="hp-menu-description">'.$description.'</div>';
				}
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				$output .= '<ul' . ( $menu_id ? ' id="' . esc_attr( $menu_id ) . '"' : '' ) . ( $menu_class ? ' class="' . esc_attr( $menu_class ) . '"' : '' ) . '>';
					$output .= '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="">' . esc_html__( 'Add a menu', 'mitsubishi-hp' ) . '</a></li>';
				$output .= '</ul>';
				if ( $container ) {
